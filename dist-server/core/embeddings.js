...
exports.findSemanticallySimilar = findSemanticallySimilar;
exports.checkEmbeddingModelAvailable = checkEmbeddingModelAvailable;
exports.listEmbeddingModels = listEmbeddingModels;
exports.getSemanticTagConnections = getSemanticTagConnections;
...
async function generateEmbedding(text) {
    ...
    const model = (0, database_1.getSetting)('embeddingModel') || DEFAULT_EMBEDDING_MODEL;
 ...
 */
async function findSemanticallySimilar(tagName, limit = 20) {
    // 1. Get all blocks tagged with this tag
    const taggedBlocks = (0, database_1.getBlocksWithTag)(tagName);
    if (taggedBlocks.length === 0) {
        return [];
    }
    // 2. Get their embeddings
    const blockIds = taggedBlocks.map((b) => b.id);
    const embeddingsMap = (0, database_1.getBlockEmbeddings)(blockIds);
    ...
    const knnResults = (0, database_1.findSimilarBlocksKNN)(centroid, limit + taggedBlocks.length);
    ...
    for (const result of knnResults) {
        ...
        const block = (0, database_1.getBlockById)(result.block_id);
        if (!block) {
 ...
 */
async function checkEmbeddingModelAvailable() {
    const endpoint = (0, database_1.getSetting)('ollamaEndpoint') || DEFAULT_OLLAMA_ENDPOINT;
    const model = (0, database_1.getSetting)('embeddingModel') || DEFAULT_EMBEDDING_MODEL;
    try {
        ...
        if (actualDim !== database_1.EMBEDDING_DIMENSION) {
            return {
                available: false,
                model,
                error: `Dimension mismatch: expected ${database_1.EMBEDDING_DIMENSION}, got ${actualDim}`,
            };
        }
...
async function listEmbeddingModels() {
    ...
    try {
        ...
        return (data.models || [])
            .map((m) => m.name)
            .filter((name) => embeddingModelPatterns.some((pattern) => name.toLowerCase().includes(pattern.toLowerCase())));
    }
    catch {
        return [];
    }
 ...
 */
function getSemanticTagConnections(cooccurrencePairs, minSimilarity = 0.7, limit = 20) {
    const allTags = (0, database_1.getTagsWithCounts)();
    if (allTags.length === 0)
        return [];
    ...
    for (const tag of allTags) {
        ...
        const blocks = (0, database_1.getBlocksWithTag)(tag.name);
        if (blocks.length === 0)
            continue;
        ...
        const similar = (0, database_1.findSimilarBlocksKNN)(centroid, 30);
        ...
        for (const result of similar) {
            ...
            const blockTags = (0, database_1.getBlockTags)(result.block_id);
