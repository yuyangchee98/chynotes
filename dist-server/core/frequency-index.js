...
exports.buildFrequencyIndex = buildFrequencyIndex;
exports.updateFrequencyIndexForNote = updateFrequencyIndexForNote;
exports.queryTermFrequency = queryTermFrequency;
exports.queryFrequentTerms = queryFrequentTerms;
exports.isFrequentTerm = isFrequentTerm;
 ...
 */
function isInsideTag(text, start, end) {
 ...
 */
function isAtLineStart(line, wordStart) {
 ...
 */
function extractProperNouns(line, noteDate) {
    const occurrences = [];
    ...
    while ((match = wordPattern.exec(line)) !== null) {
        ...
        if (isAtLineStart(line, start))
            continue;
        // Skip if inside [[tag]]
        if (isInsideTag(line, start, start + word.length))
            continue;
 ...
 */
function extractProperNounPhrases(line, noteDate) {
    const occurrences = [];
    ...
    while ((match = phrasePattern.exec(line)) !== null) {
        ...
        if (isAtLineStart(line, start))
            continue;
        // Skip if inside [[tag]]
        if (isInsideTag(line, start, start + phrase.length))
            continue;
 ...
 */
function extractTermsFromNote(content, noteDate) {
...
async function buildFrequencyIndex() {
    // Clear existing data
    (0, database_1.clearTermFrequency)();
    // Get all notes (returns Date[])
    const noteDateObjs = await (0, file_manager_1.listAllNotes)();
    const allOccurrences = [];
    for (const dateObj of noteDateObjs) {
        const noteDate = (0, file_manager_1.formatDateForFileName)(dateObj);
        const content = await (0, file_manager_1.readNote)(dateObj);
        if (content) {
    ...
    for (const agg of aggregations.values()) {
        if (agg.notes.size >= 2) {
            ...
            const existingTag = (0, database_1.getTagByName)(agg.term);
            if (existingTag)
                continue;
            (0, database_1.upsertTermFrequency)(agg.term, Array.from(agg.originalForms), agg.notes.size, agg.totalCount, Array.from(agg.notes));
            savedCount++;
        }
    }
}
 ...
 */
function updateFrequencyIndexForNote(noteDate, content) {
    // Extract terms from this note
    const noteOccurrences = extractTermsFromNote(content, noteDate);
    // Get all existing frequency data
    const existingTerms = (0, database_1.getFrequentTerms)(1); // Get all, even with noteCount=1
    ...
    for (const [term, existing] of termMap) {
        ...
        if (noteEntry && !wasInNote) {
            ...
            (0, database_1.upsertTermFrequency)(term, newForms, newNotes.length, existing.totalCount + noteEntry.count, newNotes);
        }
            ...
            if (newNotes.length === 0) {
                ...
                (0, database_1.upsertTermFrequency)(term, existing.originalForms, newNotes.length, existing.totalCount, newNotes);
            }
        }
        else if (noteEntry && wasInNote) {
            // Term still in note, might have different count/forms
            const newForms = Array.from(new Set([...existing.originalForms, ...noteEntry.originalForms]));
            (0, database_1.upsertTermFrequency)(term, newForms, existing.noteCount, existing.totalCount, existing.notes);
        }
    }
    ...
    for (const [term, entry] of noteTerms) {
        if (!termMap.has(term)) {
            // Skip if this term is already an existing tag
            const existingTag = (0, database_1.getTagByName)(term);
            if (existingTag)
                continue;
            // New term, add with noteCount=1
            (0, database_1.upsertTermFrequency)(term, Array.from(entry.originalForms), 1, entry.count, [noteDate]);
        }
    }
}
// ============================================================================
 ...
 */
function queryTermFrequency(term) {
    return (0, database_1.getTermFrequency)(term.toLowerCase());
}
/**
 * Get all frequent terms (noteCount >= 2)
 */
function queryFrequentTerms() {
    return (0, database_1.getFrequentTerms)(2);
}
/**
 * Check if a term appears frequently (in 2+ notes)
 */
function isFrequentTerm(term) {
    const freq = (0, database_1.getTermFrequency)(term.toLowerCase());
